...
var Magic = require('mmmagic').Magic; //Verify conversion
...
var WordDocumentHandler = function(opts) {
  ...
  this.overWrite = opts.overWrite === undefined ? true : opts.overWrite;
...
WordDocumentHandler.prototype.__onDocumentConvert = function(opts) {
  var resultPath = this.normalPath + '.docx';
  var magic = new Magic();
  var _self = this;
  magic.detectFile(resultPath, function(err, result) {
    _self.__onFileVerification( err, result, resultPath, opts );
  });
};

WordDocumentHandler.prototype.__onFileVerification = function(err, result, 
                                                          resultPath, opts) {
  if (err) {
    this.__onErrorConversion( resultPath );
  } else {
    if (result == 'empty') {
      this.__onErrorConversion( resultPath );
    } else {
...
WordDocumentHandler.prototype.__onErrorConversion = function(resultPath) {
  this.errors.push( new WordDocumentHandler.Error( 
